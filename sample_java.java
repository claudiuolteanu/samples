...
public class Ants {
	...
	private int max_EAST = 0;
	private int max_WEST = 0;
	private int max_SOUTH = 0;
	private int max_NORTH = 0;
	private Ilk map[][];
	...
	public int max_EAST(){
		return this.max_EAST;
	}
	
	public int max_SOUTH(){
		return this.max_SOUTH;
	}
	
	public int max_NORTH(){
		return this.max_NORTH;
	}
	
	public int max_WEST(){
		return this.max_WEST;
	}
		...
		for (String line : data) {
			...
			if (tokens.length > 2) {
				...
				if (tokens[0].equals("w")) {
			    	...
			    	t.mark = 0;
			    	this.antList.put(t, ilk);
			    	if((row + viewradius2) < max_NORTH )
					max_NORTH = row + viewradius2;
				if((row + viewradius2) > max_SOUTH)
					max_SOUTH = row + viewradius2;
				if((col + viewradius2) < max_WEST)
					max_WEST = col + viewradius2;
				if((col + viewradius2) > max_EAST)
					max_EAST = col + viewradius2;
